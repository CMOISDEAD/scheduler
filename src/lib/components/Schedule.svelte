			...
			{#each $data.days as day}
				{#if schedule.find((clase) => clase.day === day && clase.start === hour)}
					{#each schedule.filter((clase) => clase.day === day && clase.start === hour) as clase}
						<div
							class="flex flex-col content-center items-center justify-center rounded-lg border border-neutral-800 bg-neutral-900/75 p-2 transition-colors hover:cursor-pointer hover:border-neutral-700"
